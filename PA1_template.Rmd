...
The mean number of steps per day is `r mean(total)`.
```{r echo=TRUE}
mean(total)
```
The median number of steps per day is `r median(total)`.
```{r echo=TRUE}
median(total)
```
...
The total number of steps with missing values is `r sum(is.na(data$steps))`.
